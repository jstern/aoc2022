...
const reactions = {
    [win]: {[againstRock]: playPaper, [againstPaper]: playScissor, [againstScissor]: playRock},
    [lose]: {[againstRock]: playScissor, [againstPaper]: playRock, [againstScissor]: playPaper},
    [draw]: {[againstRock]: playRock, [againstPaper]: playPaper, [againstScissor]: playScissor},
};
...
export function part1(input: string): string {
    let total = 0;
    lines(input).forEach(line => { total += score(line.split(delimiter)); });
...
export function part2(input: string): string {
    let total = 0;
    lines(input).forEach(line => { total += score(reaction(line.split(delimiter))); });
    return total.toString();
}
